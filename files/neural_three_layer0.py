...
class Neural:
    def __init__(self,layer_size,input_size,hidden_size,output_size,weight_init_std=0.01):
        self.params={}
        ...
        for i in range(2,layer_size):
            self.params['W'+str(i)] = weight_init_std * np.random.randn(hidden_size,hidden_size)
            self.params['b'+str(i)] = np.zeros(hidden_size)

        self.params['W'+str(leyer_size)] = weight_init_std * np.random.randn(hidden_size,output_size)
        self.params['b'+str(layer_size)] = np.zeros(output_size)
